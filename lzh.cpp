...
/* Digital Dynamics conversion LZH (LHarc) encoding functions	*/
...
uint16_t	  lzh_match_position, lzh_match_length,
...
/******************************/
void lzh_insert_node(uint16_t r)
{
	uint16_t  i, p, cmp;
	uint8_t  *key;
	uint32_t c;
...
void lzh_delete_node(uint16_t p)  /* Deleting node from the tree */
{
	uint16_t  q;
 ...
 */
uint16_t *lzh_prnt=NULL;

/* pointing children nodes (son[], son[] + 1)*/
uint16_t *lzh_son=NULL;

#else	/* STATIC */

uint16_t lzh_freq[LZH_T + 1];   /* cumulative freq table */
uint16_t lzh_prnt[LZH_T + LZH_N_CHAR];
uint16_t lzh_son[LZH_T + 1];		  /* bug fixed by Digital Dynamics */
...
int lzh_getbit(uint8_t *inbuf, uint32_t *incnt, uint32_t inlen)    /* get one bit */
{
	int16_t i;
...
uint16_t lzh_getbyte(uint8_t *inbuf, uint32_t *incnt, uint32_t inlen)   /* get a byte */
{
	uint16_t i;
...
/* output c bits */
void lzh_putcode(uint16_t l, uint16_t c, uint8_t *outbuf, uint32_t *outlen)
{
...
void lzh_encode_char(uint16_t c, uint8_t *outbuf, uint32_t *outlen)
{
	uint16_t i;
	uint16_t j, k;
...
void lzh_encode_position(uint16_t c, uint8_t *outbuf, uint32_t *outlen)
{
	uint16_t i;

	/* output upper 6 bits with encoding */
	i = c >> 6;
	lzh_putcode(lzh_p_len[i], (uint32_t)lzh_p_code[i] << 8, outbuf, outlen);
...
void lzh_encode_end(uint8_t *outbuf, uint32_t *outlen)
{
	if (lzh_putlen) {
		outbuf[(*outlen)++]=(lzh_putbuf >> 8);
	}
...
uint16_t lzh_decode_char(uint8_t *inbuf, uint32_t *incnt, uint32_t inlen)
{
	uint16_t c;
...
uint32_t lzh_decode_position(uint8_t *inbuf, uint32_t *incnt, uint32_t inlen)
{
	uint16_t i, j, c;
...
uint32_t lzh_encode(uint8_t *inbuf, uint32_t inlen, uint8_t *outbuf)
{
	...
	uint32_t incnt,outlen; /* textsize=0; */
...
uint32_t lzh_encode(uint8_t *inbuf, uint32_t inlen, string& strRet)
{
    ...
    nOutlen = inlen < 1024 ? 2048 : (inlen * 2);
    uint8_t *outbuf = (uint8_t*) malloc (nOutlen * sizeof (uint8_t));
	if (outbuf == NULL)
    {
        fprintf (stderr, "Error: No enough memory to malloc.\n");
    ...
    strRet="";
    strRet.append ((const char*)outbuf, outlen);
    free (outbuf);
